...
* **CLI Dependency Installer Bugfix**: Fixed two critical installation/migration issues on Windows when using Bun for setup operations:
  ...
  **Resolution**: Configured the customization CLI to always build the `node_modules` folders for *both* frontend and backend using **`npm install`** and to execute migrations using **`npm run db:push`**. This guarantees a 100% stable, standard Node-compatible layout, while still enabling the Express backend to run with maximum speed under the **Bun** runtime during development and production.
